...
const roomSchema = z.object({
  ...
  capacity: z.number({ invalid_type_error: 'Capacity must be a number' }).int().positive('Capacity must be more than 0'),
  sizeSqft: z.number({ invalid_type_error: 'The area of ​​the room must be a number' }).int().positive('The room area must be more than 0'),
  facilities: z.array(z.string()).min(1, 'Required facilities'),
  imageUrl: z.string().url('Invalid image URL'),
  availableStart: z.string().regex(/^\d{2}:\d{2}$/, 'Invalid time format').optional().default('08:00'),
  availableEnd: z.string().regex(/^\d{2}:\d{2}$/, 'FInvalid time format').optional().default('18:00'),
})
...
const createRoom = async (req, res) => {
  try {
    // Parse facilities if sent as a string
    ...
    return res.status(201).json({ message: 'Room successfully added.', room })
  } catch (error) {
...
const updateRoom = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'Room successfully updated.', room })
  } catch (error) {
